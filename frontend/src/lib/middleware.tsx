...
const ProtectedRoute = () => {
      ...
      while (retries > 0) {
        try {
          ...
          if (axios.isAxiosError(err)) {
            ...
            if (retries > 0) {
              console.warn(
                "Server error. Retrying... (" + retries + " attempts left)"
              )
  ...
  useEffect(() => {
    const handleSessionError = (event: Event) => {
      ...
      if (variant == "forbidden") setForbiddenOpen(true)
      setShowDialog(true)
    }
  ...
  if (showDialog) {
    if (variant != "forbidden") {
      return (
        ...
        return (<AuthAlert isOpen={forbiddenOpen} variant={variant} onLogin={onLogin} />)
    }
  }
