...
export const RoleUpdateForm = () => {
  ...
  const form = useForm<z.infer<typeof updateRoleFormSchema>>({
    resolver: standardSchemaResolver(updateRoleFormSchema),
    defaultValues: {
      name: role.name,
  ...
  const handleUpdate = async (data: z.infer<typeof updateRoleFormSchema>) => {
    if (isLoading) throw new Error('Already creating the role...')
    if (!activeGroup?.id) throw new Error('activeGroup.id is null')
        setIsLoading(true);
    try {
        const apiCall = roleService.updateRole(activeGroup.id, role.id, data)
  ...
  async function onSubmit(data: z.infer<typeof updateRoleFormSchema>) {
